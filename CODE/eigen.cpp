...
namespace kht {
    ...
    namespace detail {
        ...
        */
        inline void tri_diagonalize(double const *Cxd, double *d, double *e, double *A, int L, double tol) {
            int i, j, k, l;
            double f, g, h, hh;
            for (i = 0; i < L; i++) {
                for (j = 0; j <= i; j++) {
        ...
        */
        ...
        void eigen(Matrix &vectors, Matrix &values, Matrix const &matrix) {
            ...
            tri_diagonalize(matrix.data(), values.data(), temp, vectors.data(), 2, 1.0e-6);
            calc_eigenstructure(values.data(), temp, vectors.data(), 2, 1.0e-16);
